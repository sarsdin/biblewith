...
//            .setFullScreenIntent(pendingIntent, true)     //풀스크린 인텐트를 걸면 중요도가 특수레벨로 올라서 알림이 꺼지지 않음
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel( channelId,"default2", NotificationManager.IMPORTANCE_HIGH) //여기 채널 id 지정(채널만들시)
        ...
        // 알림 안돼면 android.permission.POST_NOTIFICATIONS 이거 요청해야함. 안드로이드 13부터 바뀜.
...
//        Log.e(tagName, "채팅 클라이언트 시작 성공22!!")
        ...
        while (true) {
            try {
                socket = Socket(ip, 8088).apply {
                    this.soTimeout = 6000 //소켓 응답 대기 시간 6초
                }
