...
//            onNavDestinationSelected(it, navController)  << navigate()와 충돌함.
        ...
        //채팅다이얼로그 fm 을 dismiss하고 true로바꾸면 방생성이 취소됐다는 것을 알리고 groupinchatfm 채팅방목록갱신을 다시 시작함
        groupVm.liveChatCreateDialogFmIsDismiss.observe(viewLifecycleOwner, Observer {
            if(it){
                채팅방목록갱신()
                groupVm.liveChatCreateDialogFmIsDismiss.value = false
            }
        })
